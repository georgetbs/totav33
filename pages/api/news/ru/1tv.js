// pages/api/news/ru/1tv.js
import fetch from 'node-fetch';
import newsCache from '../../../../lib/cache';

const BASE_URL = "https://1tv.ge/lang/wp-json/listing/all/news?offset=0&lang=ru&post_type=news&topic=&tpl_ver=42.0.1";
const CACHE_KEY = "news1tvMainRu";

export default async function handler(req, res) {
  // Проверка кэша данных
  const cachedData = newsCache.get(CACHE_KEY);
  if (cachedData) {
    console.log(`Returning cached data for 1tv Main Ru`);
  ...
  try {
    ...
    const newsItems = data.map(item => ({
      title: item.post_title,  // Убираем decodeURIComponent
